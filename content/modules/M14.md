---
id: "M14"
title: "Ertragsoptimierung pro Kunde"
subtitle: "Kundendeckungsbeitrag analysieren, risikogerechtes Pricing verstehen und Konditionengespräche wertbasiert führen"
kompetenzfeld: "K-04 Vertrieb & Ertragsoptimierung"
kompetenzfeld_slug: "vertrieb"
stufe: "Sparring"
bloom: "3–5"
dauer: "90 Min."
format: "Workshop mit Rechenübungen (Präsenz)"
status: "freigegeben"
version: "v0.1"
voraussetzungen: ["M13"]
folgemodule: ["M15", "M16"]
youtube_id: ""
praxis_review: false
wiss_review: false
lernziele:
  - text: "Den Kundendeckungsbeitrag anhand der vier Ertragskomponenten berechnen"
    bloom_stufe: 3
  - text: "Die Ertragsstruktur eines Kunden analysieren und die größten Optimierungshebel identifizieren"
    bloom_stufe: 4
  - text: "Den Expected Loss in der Konditionenkalkulation erklären und bewerten, ob eine Beziehung risikogerecht bepreist ist"
    bloom_stufe: 5
  - text: "Drei Optimierungsszenarien bewerten und die ertragswirksamste Option wählen"
    bloom_stufe: 5
  - text: "Ein Konditionengespräch wertbasiert führen – Nutzen kommunizieren bevor über Konditionen gesprochen wird"
    bloom_stufe: 3
---

> *Stand: v0.1 · Benedikt Zoller Coaching*


---

## Modulinhalte

### Warum Ertrag je Kunde messen?

Die zeb-Firmenkundenstudie zeigt, dass in einem typischen VR-Bank-Firmenkundenbestand 20 % der Kunden 80 % des Deckungsbeitrags erzeugen. Ohne eine klare Ertragsanalyse investieren Berater Zeit und Kapital in Kunden, die keinen oder negativen Deckungsbeitrag liefern – während ertragstarke Kunden möglicherweise zu wenig Aufmerksamkeit erhalten.

Schierenbeck et al. definieren die Kunden-Deckungsbeitrag-Rechnung als das zentrale Steuerungsinstrument der kundenbezogenen Rentabilitätsanalyse im Bankgeschäft. Sie erfasst alle Ertrags- und Kostengrößen, die einem einzelnen Kunden direkt zugeordnet werden können.

### Die vier Ertragskomponenten

Der Kundendeckungsbeitrag setzt sich zusammen aus:

**Deckungsbeitrag I = Zinsüberschuss + Provisionsertrag + Verbundertrag − Risikokosten**

**Deckungsbeitrag II = DB I − Betriebskosten (Betreuungsaufwand)**

| Komponente | Beschreibung | Typische Treiber |
|-----------|-------------|-----------------|
| **Zinsüberschuss** | Zinsmarge aus Kredit- und Einlagengeschäft | Konditionshöhe, Volumen, Laufzeit |
| **Provisionsertrag** | Gebühren aus ZV, Avale, Dokumenten, Beratung | Zahlungsverkehrsvolumen, Produktkomplexität |
| **Verbundertrag** | Provisionen aus Verbundprodukten (R+V, Union Investment, etc.) | Cross-Selling-Tiefe, Verbundpartner-Einbindung |
| **Risikokosten** | Expected Loss (EL = PD × LGD × EAD) | Ratingklasse, Besicherung, Kreditvolumen |
| **Betriebskosten** | Direkter Betreuungsaufwand (Std. × Kostensatz) | Gesprächsfrequenz, Komplexität, Sonderleistungen |

### Rechenbeispiel: Kundendeckungsbeitrag Müller GmbH

| Position | Betrag p.a. |
|----------|------------|
| Kreditzinsen (Marge 1,85 % auf 800.000 EUR) | 14.800 EUR |
| Einlagenzinsen (Marge 0,45 % auf 300.000 EUR) | 1.350 EUR |
| Provisionsertrag ZV, Avale, Beratung | 3.200 EUR |
| Verbundertrag (R+V Prämie, keine UI-Verbindung) | 1.100 EUR |
| **Bruttoertrag** | **20.450 EUR** |
| ./. Risikokosten (Rating B, EL 0,8 % × 800.000) | − 6.400 EUR |
| **Deckungsbeitrag I** | **14.050 EUR** |
| ./. Betriebskosten (12 Std. × 120 EUR/Std.*) | − 1.440 EUR |
| **Deckungsbeitrag II** | **12.610 EUR** |

**Ertragskennzahl:** DB II / Kreditvolumen = 1,58 % (Kundenrendite)

*\* Der Kostensatz von 120 EUR/Std. ist ein Richtwert für die Übung. Der in Ihrem Haus gültige Wert kann abweichen (typische Bandbreite: 90–150 EUR je nach Bankgröße und Kostenstruktur). Fragen Sie Ihr Controlling nach dem hausindividuellen Vollkostensatz (agree Vollkostenrechnung oder internes Benchmark-Controlling).*

> **agree-Navigation für DB-Auswertungen:** Je nach Bankaufstellung unterschiedlich hinterlegt. Einstiegspunkt: **agree → Kundenakte → Ertragskennzahlen → Deckungsbeitragsauswertung (KSC)**. Alternativ: agree → Vertriebssteuerung → Ertragskennzahl-Cockpit. Falls nicht auffindbar: Controlling oder IT-Koordinator fragen – viele Häuser haben hausindividuelle Menüpfade konfiguriert.

## 4.2 Risikogerechtes Pricing

### Der Expected Loss als Kalkulationsgrundlage

In der Konditionenkalkulation muss die Risikoprämie mindestens den Expected Loss (EL) des Kreditengagements abdecken. Der EL errechnet sich nach Basel III aus:

**EL = PD × LGD × EAD**

| Parameter | Bedeutung | Praktische Annäherung |
|-----------|----------|----------------------|
| **PD** (Probability of Default) | Ausfallwahrscheinlichkeit | Abgeleitet aus dem internen Rating; VR-Banken nutzen VR-Rating/agree |
| **LGD** (Loss Given Default) | Verlust bei Ausfall | Abhängig von Besicherung; typisch 40–60 % unbesichert |
| **EAD** (Exposure at Default) | Kreditvolumen bei Ausfall | Inanspruchnahme + konservative Schätzung Inanspruchnahme offener Linien |

**Vereinfachungsregel im Beratungsalltag:** Für eine Überschlagsrechnung gilt bei durchschnittlichem Firmenkundenrating (BB/B-Klasse) eine EL-Spanne von 0,5–1,5 % p.a. auf das Kreditvolumen. Liegt die vereinbarte Kreditmarge unterhalb des EL, subventioniert die Bank das Engagement.

### Das Nettomargenziel (NMZ) als Steuerungsgröße

In vielen VR-Banken wird das **Nettomargenziel (NMZ)** als Untergrenze für das Konditionensetting genutzt. Das NMZ gibt an, welche Nettomarge nach Abzug von Refinanzierungskosten, Risikokosten (EL) und Standardbetriebskosten mindestens verbleiben muss, damit eine Kreditbeziehung als tragfähig gilt.

**Logik:**

> Bruttomarge (vereinbarte Kreditmarge) − Refinanzierungskosten − EL − Standardbetriebskosten = **Nettomarge**

Liegt die Nettomarge unterhalb des hausindividuellen NMZ, ist eine Konditionsanpassung betriebswirtschaftlich zwingend – unabhängig davon, ob der Kunde zufrieden ist oder nicht. Die NMZ wird im DZ BANK Pricing-Leitfaden und in agree als Mindest-Benchmark hinterlegt. **Vertiefung → M16 NMZ-Optimierung** (Ertragsteuerung auf Bestandsebene).

### Konditionsanpassung vorbereiten

Wenn die Analyse zeigt, dass ein Kunde unter dem Mindest-Deckungsbeitrag liegt, gibt es drei Hebel:

1. **Zinsmarge erhöhen**: Direkte Wirkung auf DB I. Erfordert Konditionengespräch (Sec 4.4).
2. **Cross-Selling ausbauen**: Verbundertrag und Provisionen steigern, ohne Zinskonditionen anzufassen. Oft der schonendere Weg.
3. **Betreuungskosten senken**: Gesprächsfrequenz anpassen, digitale Kanäle stärker nutzen. Wirkt auf DB II.

Rolfes betont, dass nachhaltige Ertragsoptimierung alle drei Hebel kombiniert – ein reiner Konditionsfokus führt häufig zur Abwanderung des Kunden, ohne den Deckungsbeitrag dauerhaft zu verbessern.

> **Wichtig – Genehmigungspflicht beachten:** Konditionsanpassungen oberhalb des eigenen Kompetenzrahmens erfordern ein Votum der Marktfolge oder die Freigabe durch den Bereichsleiter. Kläre **vor** dem Kundengespräch intern, welchen Spielraum du hast. Wer dem Kunden eine Konditionen-Zusage macht und diese dann nicht genehmigt bekommt, beschädigt seine Glaubwürdigkeit dauerhaft.

## 4.3 Ertragsstruktur-Analyse im Bestand

### Kundensegmentierung nach Ertragsprofil

Eine einfache 2×2-Matrix hilft, den Bestand nach Ertragspotenzial und aktuellem Ertrag zu strukturieren:

| | **Hoher aktueller DB** | **Niedriger aktueller DB** |
|---|---|---|
| **Hohes Potenzial** | Kernkunde: halten, ausbauen | Entwicklungskunde: Cross-Selling, Konditionen |
| **Niedriges Potenzial** | Ertragskunde: Beziehung pflegen | Problemkunde: Betreuungsintensität senken oder Exit prüfen |

**Kernkunden** (hoher DB, hohes Potenzial) erhalten bevorzugte Betreuungsintensität: proaktive Kontakte, Spezialisten einbinden, Strategiegespräche.

**Entwicklungskunden** (niedriger DB, hohes Potenzial) sind die Priorität für Ertragsoptimierung: Hier liegt ungenutztes Wallet-Share-Potenzial.

**Problemkunden** (niedriger DB, niedriges Potenzial) sollten mit minimalem Aufwand betreut werden. Exit aus der Beziehung ist bei dauerhaft negativem DB zu prüfen – aber:

> **Exit-Entscheidungen sind immer Chefsache.** In einer regionalen Volksbank sind viele „Problemkunden" Genossenschaftsmitglieder, regional vernetzt, oder haben Lieferanten- und Abnehmerbeziehungen zu anderen Kunden im Bestand. Ein unvorbereiteter Exit kann erhebliche Reputationsschäden und Folgewirkungen auslösen. Der Berater analysiert und empfiehlt – die Entscheidung liegt bei Bereichsleitung oder Vorstand. Genossenschaftliche Mitgliedschaft, regionale Vernetzung und Reputationswirkung müssen immer abgewogen werden.

### Signale für Unterpreisierung

Berater erkennen unterpreiste Engagements oft an folgenden Signalen:
- Kreditmarge liegt deutlich unter Marktüblichkeit (Benchmarkcheck in agree)
- Kein Verbundpartner eingebunden, obwohl Potenzial vorhanden
- Hohe Inanspruchnahme offener Linien → steigt EAD, Marge wurde nie angepasst
- Ratingverschlechterung ohne Konditionsanpassung → EL gestiegen, Marge unverändert

## 4.4 Das Konditionengespräch wertbasiert führen

### Vom Konditionengespräch zum Wertgespräch

Das klassische Konditionengespräch beginnt mit Zahlen: „Ihr Zinssatz beträgt X, und wir möchten auf Y anpassen." Diese Eröffnung setzt den Kunden sofort in eine Verhandlungsposition – er denkt in Basispunkten, nicht in Wert.

Ein wertbasierter Ansatz kehrt die Reihenfolge um:

**Phase 1: Wert der Beziehung artikulieren**
Bevor Konditionen angesprochen werden, wird der Nutzen der Bankbeziehung konkret gemacht: schnelle Kreditentscheidungen, Verbundpartner-Einbindung, Beratungsleistung, Branchenwissen. „Was hat unsere Zusammenarbeit im letzten Jahr für Sie gebracht?"

**Phase 2: Marktumfeld erklären**
Zinsentwicklung, Refinanzierungskosten, Risikoumfeld transparent machen – ohne Details zu internen Kalkulationen preiszugeben.

*Musterdialog:* „Im aktuellen Marktumfeld – gestiegene Rohstoffpreise, unsicherere Lieferketten – sind die Ausfallrisiken im Mittelstand insgesamt gestiegen. Das spiegelt auch unser Rating-Prozess wider. Ich möchte fair mit Ihnen sein und Ihnen erklären, warum wir eine Anpassung vorsehen – und welche Optionen wir dabei haben."

Diese Formulierung erklärt das Warum, ohne interne Kalkulation offenzulegen, und signalisiert Partnerschaft statt Diktat. Der Satz „Ich möchte fair mit Ihnen sein" ist ein Vertrauenssignal, das die Verhandlungsposition des Beraters stärkt.

**Phase 3: Konditionen transparent kommunizieren**
Erst jetzt die konkrete Anpassung benennen – mit Begründung, nicht als einseitige Mitteilung. „Auf Basis dieser Entwicklung möchten wir die Marge um X Basispunkte anpassen. Was ist Ihre erste Reaktion?"

**Phase 4: Alternativen anbieten**
Dem Kunden Optionen geben: „Wenn wir die Besicherung verbessern, könnten wir bei der Marge etwas flexibler sein." Oder: „Wenn Sie das R+V-Paket hinzunehmen, sehen wir die Gesamtbeziehung anders."

## 4.5 Praxiscase: Ertragsanalyse Bauer & Söhne GmbH

**Ausgangssituation**

Beraterin Sandra Köhler betreut die Bauer & Söhne GmbH, einen Holzhandel mit 19 Mitarbeitern und einem Jahresumsatz von 6,8 Mio. EUR. Die Beziehung besteht seit 12 Jahren. Der Gesellschafter Günter Bauer ist seit Jahren ein loyaler Kunde – nie Ärger, nie Sonderwünsche.

Bei der Vorbereitung des Jahresgesprächs analysiert Sandra den Deckungsbeitrag:

| Position | Betrag p.a. |
|----------|------------|
| Kreditmarge (1,25 % auf 1,2 Mio. EUR) | 15.000 EUR |
| Einlagenmarge (0,30 % auf 180.000 EUR) | 540 EUR |
| ZV-Provisionen | 2.100 EUR |
| Verbundertrag | 0 EUR |
| **Bruttoertrag** | **17.640 EUR** |
| ./. Risikokosten (Rating B+, EL 0,65 % × 1.200.000) | − 7.800 EUR |
| **DB I** | **9.840 EUR** |
| ./. Betriebskosten (15 Std. × 120 EUR) | − 1.800 EUR |
| **DB II** | **8.040 EUR** |
| Kundenrendite (DB II / Kreditvolumen) | **0,67 %** |

**Die Analyse zeigt drei Probleme:**
1. Die Kreditmarge von 1,25 % deckt den EL von 0,65 % nur mit 0,60 % Nettomarge – unter dem bankinternen Mindeststandard von 0,80 % (fiktiver Fallbeispielwert; die konkrete Untergrenze wird hausindividuell festgelegt).
2. Kein Verbundpartner eingebunden – obwohl Günter Bauer drei Mitarbeiter hat und eine Betriebsunterbrechungsversicherung naheliegt.
3. Der Betreuungsaufwand von 15 Stunden ist für das Ertragsniveau hoch.

**Sandras Optimierungsstrategie**

Sandra entscheidet: keine direkte Konditionsanpassung als Einstieg. Stattdessen:
1. R+V Gewerbeberater einladen: Betriebsunterbrechung + Schlüsselpersonenversicherung → Verbundertrag + 1.800 EUR
2. Auslands-ZV prüfen: Günter importiert Holz aus Österreich und Polen → Provisionen + 600 EUR
3. Bei nächster Kreditverlängerung (in 8 Monaten): Marge auf 1,45 % anpassen, Begründung: Marktumfeld + Rating

**Simulierter DB nach Umsetzung – zwei Szenarien:**

| Szenario | Maßnahmen | DB II | Veränderung | Kundenrendite |
|----------|-----------|------:|:-----------:|:-------------:|
| **A – kurzfristig** (sofort umsetzbar) | R+V Verbundertrag +1.800 EUR; Auslands-ZV +600 EUR | **10.440 EUR** | +30 % | 0,87 % |
| **B – nach Kreditverlängerung** (in 8 Monaten) | Szenario A + Margenanpassung auf 1,45 % (+2.400 EUR) | **12.840 EUR** | +60 % | 1,07 % |

**Proaktive Kundenbindung und Fallback-Szenarien**

Sandra trägt zwei zusätzliche Maßnahmen in ihre agree-Wiedervorlage ein:
- **3 Monate vor der Kreditverlängerung** (also in 5 Monaten): Jahresgespräch einplanen – Fokus auf Beziehungsstärke, nicht auf Zahlen. Ziel: Günter soll spüren, dass Sandra die Beziehung aktiv pflegt, bevor die Konditionsdiskussion kommt.
- **Fallback wenn Günter die R+V-Einladung ablehnt:** Sandra notiert in agree als Alternative: Schlüsselpersonenversicherung als niedrigschwelligen Einstieg (kleineres Volumen, überschaubare Prämie) oder ZV-Optimierung als alleinige erste Maßnahme. Kein Cross-Selling-Potenzial geht verloren – es verschiebt sich nur.

**Reflexionsfragen**

1. Warum war es sinnvoll, die Konditionsanpassung auf später zu verschieben statt sofort anzusprechen?
2. Welche der drei Optimierungsmaßnahmen hat die größte Hebelwirkung auf den DB – und warum?
3. Was würde Sandra tun, wenn eine Konkurrenzbank Günter Bauer mit einem günstigeren Angebot kontaktiert, bevor die Kreditverlängerung ansteht?
4. Welcher Ihrer aktuellen Kunden könnte ein ähnliches „unsichtbares Ertragsproblem" haben?

## 4.6 Arbeitsblätter

### Arbeitsblatt A: Deckungsbeitragsrechnung

**Kundenname (anonymisiert):** ___________________________ **Analysedatum:** _______________

| Position | Betrag p.a. (EUR) |
|----------|:-----------------:|
| Kreditmarge ( ___ % × ___ EUR) | |
| Einlagenmarge ( ___ % × ___ EUR) | |
| Provisionsertrag | |
| Verbundertrag | |
| **Bruttoertrag** | |
| ./. Risikokosten (EL: ___ % × ___ EUR) | |
| **Deckungsbeitrag I** | |
| ./. Betriebskosten ( ___ Std. × 120 EUR) | |
| **Deckungsbeitrag II** | |
| Kundenrendite (DB II / Kreditvolumen) | **___ %** |

**Optimierungshebel (Top 2):**

1. ___________________________________________________________________________

2. ___________________________________________________________________________

### Arbeitsblatt B: Konditionengespräch vorbereiten

**Wertargumente (Phase 1) – Was hat unsere Beziehung diesem Kunden gebracht?**

___________________________________________________________________________

**Marktumfeld-Erklärung (Phase 2) – Welche externen Faktoren begründen die Anpassung?**

___________________________________________________________________________

**Konkrete Anpassung (Phase 3):**

___________________________________________________________________________

**Alternativen (Phase 4) – Welche Optionen biete ich an?**

___________________________________________________________________________

---

---

## Praxistransfer

**Aufgabe (in den nächsten 4 Wochen):**

Wählen Sie **drei Kunden** aus Ihrem Bestand, die Ihnen hinsichtlich des Ertragsprofils unklar sind. Führen Sie für jeden die Deckungsbeitragsrechnung (Arbeitsblatt A) durch. Nutzen Sie dazu agree (Kreditakten → Ertragskennzahlen → KSC, Provisionsauswertungen, Verbundpartner-Cockpit) und eigene Schätzungen, wo Daten fehlen. **Wichtig:** Kennzeichnen Sie Schätzwerte im Arbeitsblatt mit dem Kürzel (S) und klären Sie diese vor Entscheidungen mit Ihrem Teamleiter oder dem Controlling ab. Entscheidungen auf Basis ungeprüfter Schätzungen können zu Fehlsteuerung führen.

Für **einen dieser Kunden** entwickeln Sie eine konkrete Optimierungsstrategie (mind. zwei Maßnahmen) und bereiten das nächste Gespräch mit Arbeitsblatt B vor.

**Stellen Sie die Analyse (anonymisiert) in der nächsten Teambesprechung vor** – 5 Minuten, Fokus: Was hat Sie überrascht? Welche Maßnahme planen Sie?

## 5.2 Selbstcheck nach 4 Wochen

| Aussage | Trifft voll zu (4) | Trifft überwiegend zu (3) | Trifft eher nicht zu (2) | Trifft nicht zu (1) |
|---------|:-:|:-:|:-:|:-:|
| Ich kann den Kundendeckungsbeitrag (DB I und DB II) für meine Kunden berechnen. | ☐ | ☐ | ☐ | ☐ |
| Ich verstehe, wie der Expected Loss in die Konditionenkalkulation einfließt. | ☐ | ☐ | ☐ | ☐ |
| Ich habe für mind. 3 Kunden eine DB-Analyse durchgeführt. | ☐ | ☐ | ☐ | ☐ |
| Ich kann Optimierungshebel (Zins, Cross-Selling, Kosten) für einen konkreten Kunden benennen. | ☐ | ☐ | ☐ | ☐ |
| Ich kann ein Konditionengespräch wertbasiert (Nutzen zuerst) vorbereiten und führen. | ☐ | ☐ | ☐ | ☐ |
| Ich kenne den Unterschied zwischen meinen Kernkunden, Entwicklungskunden und Problemkunden. | ☐ | ☐ | ☐ | ☐ |

**Auswertung:**
- 20–24 Punkte: Sehr guter Transfer – Kompetenzstufe M14 erreicht
- 14–19 Punkte: Grundlagen vorhanden – DB-Analyse weiter anwenden, bis sie routiniert sitzt
- unter 14 Punkte: Vertiefung empfohlen: Zahlenübung mit Trainer oder erfahrenem Kollegen

## 5.3 Vertiefungsliteratur

### Fachlich

- **Schierenbeck, H., Lister, M. & Kirmße, S. (2014)**: *Ertragsorientiertes Bankmanagement, Band 1 und 2* – Standardwerk zur Kunden-DB-Rechnung und Marktzinsmethode
- **Rolfes, B. (1999)**: *Gesamtbanksteuerung* – Konzepte des Bankcontrollings, Rentabilitätssteuerung

### Praxismaterialien

- **agree Handbuch Ertragskennzahlen**: Dokumentation zur DB-Auswertung im Firmenkundensegment (Intranet Atruvia)
- **DZ BANK Pricing-Leitfaden**: Konditionengestaltung und Mindestrisikoprämien im Firmenkundengeschäft (Verbund-Intranet)
- **zeb Firmenkundenstudie (2020)**: Benchmarks für Wallet-Share und Ertragskennzahlen

---

---

# 7. QUELLENVERZEICHNIS

## Didaktik & Kompetenzentwicklung

Anderson, L.W. & Krathwohl, D.R. (Hrsg.). (2001). *A Taxonomy for Learning, Teaching, and Assessing: A Revision of Bloom's Taxonomy of Educational Objectives*. Addison Wesley Longman.

Dreyfus, H.L. & Dreyfus, S.E. (1986). *Mind over Machine: The Power of Human Intuition and Expertise in the Era of the Computer*. Free Press.

## Bankcontrolling & Ertragsorientierung

Rolfes, B. (1999). *Gesamtbanksteuerung: Grundlagen, Konzepte und Methoden des Bankcontrollings*. Schäffer-Poeschel.

Schierenbeck, H., Lister, M. & Kirmße, S. (2014). *Ertragsorientiertes Bankmanagement: Band 1: Grundlagen, Marktzinsmethode und Rentabilitäts-Controlling* (9. Aufl.). Springer Gabler.

Schierenbeck, H., Lister, M. & Kirmße, S. (2014). *Ertragsorientiertes Bankmanagement: Band 2: Risiko-Controlling und integrierte Rendite-/Risikosteuerung* (9. Aufl.). Springer Gabler.

## Regulatorik

Basel Committee on Banking Supervision. (2011). *Basel III: A global regulatory framework for more resilient banks and banking systems* (rev. ed.). Bank for International Settlements.

## Bankfachliche Quellen

zeb consulting. (2020). *Firmenkundenstudie 8.0*. zeb.

<!-- CONTENT_THEORIE_START -->

## Wissenschaftliche Einordnung

### Marktzinsmethode und Deckungsbeitrag als analytische Grundlage

Die wissenschaftliche Grundlage der Ertragsoptimierung im Firmenkundengeschäft bildet das ertragsorientierte Bankmanagement. Schierenbeck, Lister und Kirmße (2014) entwickeln mit der Marktzinsmethode einen Rahmen, der es ermöglicht, den Beitrag einzelner Kundengeschäfte zum Gesamterfolg der Bank transparent zu machen: Jedes Produkt wird zu Opportunitätskostensätzen des Kapitalmarkts bewertet, sodass Zinsmargen, Provisionen und Risikokosten vergleichbar werden. Im Ergebnis entsteht ein Kundendeckungsbeitrag, der Grundlage für Pricing-Entscheidungen, Cross-Selling-Prioritäten und Verhandlungsführung ist.

Rolfes (1999) ergänzt dies mit dem Konzept der Gesamtbanksteuerung: Eine Bank, die einzelne Kreditgeschäfte nicht auf Ebene des Kundenbeitrags steuert, kann systematisch Verlustgeschäfte eingehen, ohne es zu merken – weil Provisionseinnahmen aus anderen Produkten den negativen Deckungsbeitrag eines unterpriceten Kredits überdecken. Für den Berater bedeutet das: Wer nur den Zinssatz kennt, nicht aber den Gesamtdeckungsbeitrag seines Kunden, ist nicht in der Lage, Konditionen informiert zu verhandeln.

### Risikokosten und Expected Loss

Die regulatorische Anforderung an risikoadjustiertes Pricing – wesentlicher Bestandteil von Basel III (BCBS, 2011) – macht die Kenntnis des Expected Loss (EL = PD × LGD × EAD) zu einer Kernkompetenz des Firmenkundenberaters. Ein Kredit, dessen Kondition den EL nicht abdeckt, ist kein Ertragsgeschäft, sondern ein subventioniertes Kundengeschäft. Schierenbeck et al. (2014, Band 2) zeigen, wie Risikokosten in die Einzelgeschäftskalkulation integriert werden – von der Bonitätseinstufung über die Ausfallwahrscheinlichkeit bis zur Verlustquote bei Ausfall.

Im M14-Kontext lernen Berater nicht, diese Kalkulation selbst von Grund auf durchzuführen, aber sie sollen verstehen, wie der von agree oder dem internen System ausgewiesene Richtsatz entstanden ist – und warum Abweichungen begründet werden müssen.

## 2.2 Einordnung in das Dreyfus-Stufenmodell

M14 baut auf M13 (Stufe 2) auf und entwickelt die Kompetenz zur Dreyfus-Stufe 3 (Kompetent) weiter (Dreyfus & Dreyfus, 1986).

| Merkmal | Stufe 2 – Fortgeschrittener Anfänger (M13) | Stufe 3 – Kompetenter Praktiker (M14) |
|---------|---------------------------------------------|----------------------------------------|
| Ertragsverständnis | Kennt Produktmargen; handelt nach Preisliste | Berechnet Kundendeckungsbeitrag; erkennt Optimierungshebel |
| Konditionsverhandlung | Reagiert auf Kundenwunsch nach Konditionen | Führt wertbasiertes Konditionsgespräch; begründet Pricing |
| Risikobeurteilung | Kennt Bonität; setzt Richtsatz um | Verknüpft Risikokosten mit Ertrag; beurteilt Rentabilität der Beziehung |
| Optimierungshorizont | Einzelprodukt | Gesamtkundenbeziehung: Zins + Provision + Verbund |

*(Dreyfus & Dreyfus, 1986)*

## 2.3 Didaktische Begründung: Workshop mit Rechenübungen

Bloom-Stufen 3–5 (Anwenden, Analysieren, Evaluieren) erfordern nach Anderson und Krathwohl (2001) nicht nur konzeptuelles Verständnis, sondern aktive Auseinandersetzung mit konkreten Problemen. Die Rechenübungen in M14 sind kein Selbstzweck: Sie überführen das abstrakte Konzept „Deckungsbeitrag" in ein handhabbares Instrument. Erst wenn der Berater einen Deckungsbeitrag selbst berechnet hat, versteht er intuitiv, warum eine 10-Basispunkte-Konditionssenkung den Gesamtertrag der Beziehung um X% reduziert. Diese Einsicht verändert die Gesprächsführung bei Konditionsverhandlungen nachhaltig.

---

<!-- CONTENT_THEORIE_END -->

<!-- CONTENT_TRAINER_START -->

## Trainerhandbuch

| Zeit | Phase | Methode | Ziel |
|------|-------|---------|------|
| 0–5 min | Einstieg & Zielklärung | Plenum, offene Frage | Erfahrungen mit Konditionsgesprächen aktivieren |
| 5–25 min | Theorie-Input: Kundendeckungsbeitrag + Ertragskomponenten | Kurzinput + Flipchart | Vier-Komponenten-Modell verstehen (Sec 4.1) |
| 25–45 min | Rechenübung 1: Deckungsbeitrag berechnen | Einzelarbeit + Auswertung | DB-Kalkulation auf Falldaten anwenden (Sec 4.2) |
| 45–55 min | Theorie-Input: Expected Loss und risikoadjustiertes Pricing | Kurzinput | EL = PD × LGD × EAD; Konditionsuntergrenze (Sec 4.3) |
| 55–70 min | Rechenübung 2: Optimierungsszenarien | Partnerarbeit | Drei Hebel vergleichen; beste Option wählen (Sec 4.4) |
| 70–82 min | Rollenspiel: Konditionsgespräch wertbasiert | Paare | Nutzen kommunizieren vor Konditionen (Sec 4.5) |
| 82–88 min | Plenumreflexion | Plenum | Erkenntnisse sichern; typische Fehler benennen |
| 88–90 min | Transfer & Abschluss | Einzelarbeit | Einen konkreten Kunden für DB-Analyse benennen |

## 3.2 Moderationshinweise

### Einstieg (0–5 min)

Eröffnungsfrage: „Stellen Sie sich vor, ein Kunde kommt und sagt: ‚Ich habe jetzt ein besseres Angebot von einer anderen Bank – 15 Basispunkte günstiger.' Was sagen Sie?"

Typische Antworten: „Ich schaue, was wir machen können" oder „Ich frage den Vorgesetzten". Ziel: Sichtbar machen, dass viele Berater Konditionsgespräche ohne Wissensgrundlage führen. Das schafft Motivation für die folgenden Rechenübungen.

### Rechenübungen (25–45 min und 55–70 min)

> **Trainer-Hinweis:** Rechenübungen explizit als Werkzeugtraining rahmen, nicht als Prüfung. „Sie sollen nicht Buchhalter werden – Sie sollen verstehen, was das Ergebnis bedeutet." Bei der Auswertung nicht nur die Zahl zeigen, sondern immer die Implikation: „Was bedeutet dieses Ergebnis für das Konditionsgespräch von morgen?"

Taschenrechner oder Smartphone erlaubt. Wer Zahlenmaterial nicht aus dem Kopf kennt: Musterprofil aus Sec 4 nutzen.

### Rollenspiel Konditionsgespräch (70–82 min)

Paare: Berater führt ein Konditionsgespräch. Regel: Nutzen der Bankbeziehung muss vor dem ersten Konditionsangebot kommuniziert werden. Beobachter: Wurde die Nutzenargumentation eingesetzt – oder wurde sofort nachgegeben?

> **Trainer-Hinweis:** Häufigster Fehler: Berater nennt sofort eine Gegenzahl (Gegenkondition), ohne den Wert der Beziehung zu kommunizieren. Das ist das Kernlernziel von LZ-5 – erst benennen, dann verhandeln.

### Erwartete Teilnehmerreaktionen

| Reaktion | Umgang |
|----------|--------|
| „Die Zahlen kenne ich gar nicht aus agree" | „Genau das ist der Punkt. Welche Information müssten Sie abrufen – und wie?" |
| „Das entscheidet doch die Marktfolge" | „Die Marktfolge entscheidet mit. Aber Sie sind im Gespräch – und Sie brauchen Argumente." |
| Rechenübung ergibt negativen DB | „Was bedeutet das? Und was würden Sie jetzt tun?" – offene Diskussion statt Musterlösung |
| „Ich kann meinem Kunden nicht sagen, wie viel er uns bringt" | „Stimmt. Aber Sie können sagen, welchen Wert Sie ihm bieten – das ist das Gegenstück." |

## 3.3 Materialien

| Material | Anzahl | Hinweis |
|----------|--------|---------|
| Rechenaufgabe DB-Kalkulation (Sec 4.2) | 13 + Reserve | Mit Leerfeldern zum Ausfüllen |
| Rechenaufgabe Optimierungsszenarien (Sec 4.4) | 13 + Reserve | Drei Szenarien vergleichen |
| Musterlösungen (Trainer) | 1 Set | Nur für Trainer; nicht vorab austeilen |
| Rollenspiel-Profil Konditionsgespräch (Sec 4.5) | 13 + Reserve | Kundenprofil + Ausgangssituation |
| Taschenrechner | 6–8 | Reserve für TN ohne Gerät |

## 3.4 Trainer-Checkliste

**Vor dem Seminar:**
- [ ] Rechenaufgaben durchgerechnet und Musterlösungen geprüft
- [ ] Raumaufstellung: Paare oder Dreiergruppen
- [ ] Einstiegsfrage vorbereitet und eigene Antwort überlegt

**Während des Seminars:**
- [ ] Rechenübungen als Werkzeugtraining gerahmt, nicht als Prüfung
- [ ] Bei Ergebnis-Divergenzen: Beide Rechenwege gezeigt und Ursache diskutiert
- [ ] Rollenspiel: Nutzenargumentation vor Konditionsnennung eingefordert

**Abschluss:**
- [ ] Jeder TN hat einen konkreten Kunden benannt, für den er die DB-Analyse macht
- [ ] Selbstcheck Sec 5 besprochen

---

---

## Evaluation

*Auszufüllen direkt nach dem Workshop (ca. 5 Minuten)*

**Modul:** M14 – Ertragsoptimierung pro Kunde | **Datum:** _______________ | **Trainer:** _______________

| Nr | Aussage | Trifft voll zu (4) | Trifft überwiegend zu (3) | Trifft eher nicht zu (2) | Trifft nicht zu (1) |
|----|---------|:-:|:-:|:-:|:-:|
| 1 | Der Workshop hat mir ein klareres Bild vom Ertrag meiner Kundenbeziehungen gegeben. | ☐ | ☐ | ☐ | ☐ |
| 2 | Das Konzept Kundendeckungsbeitrag ist verständlich und praxisrelevant. | ☐ | ☐ | ☐ | ☐ |
| 3 | Die Rechenübungen haben meinen Blick auf Konditionsgespräche verändert. | ☐ | ☐ | ☐ | ☐ |
| 4 | Das Expected-Loss-Konzept ist nachvollziehbar erklärt worden. | ☐ | ☐ | ☐ | ☐ |
| 5 | Das Rollenspiel Konditionsgespräch war realistisch und lehrreich. | ☐ | ☐ | ☐ | ☐ |
| 6 | Ich weiß jetzt, wie ich Konditionsgespräche wertbasiert führe. | ☐ | ☐ | ☐ | ☐ |
| 7 | Die drei Optimierungsszenarien (Konditionen, Cross-Selling, Kosten) sind klar unterscheidbar. | ☐ | ☐ | ☐ | ☐ |
| 8 | Tempo und Tiefe des Workshops haben gepasst. | ☐ | ☐ | ☐ | ☐ |
| 9 | Der Trainer hat Beispiele aus dem VR-Banken-Alltag eingebracht. | ☐ | ☐ | ☐ | ☐ |
| 10 | Ich empfehle diesen Workshop meinen Kollegen. | ☐ | ☐ | ☐ | ☐ |

**Was hat Ihnen am meisten gebracht?** ___________________________________________________________________________

**Was würden Sie ändern?** ___________________________________________________________________________

> **Auswertungshinweis (Trainer):** 35–40 Pkt. = sehr gute Akzeptanz | 28–34 = solide | unter 28 = Nachgespräch empfohlen.

---

## 6.2 Kirkpatrick Level 2 – Lernen (Wissenstest)

*Bearbeitungszeit: ca. 12 Minuten*

**F1:** Aus welchen vier Komponenten setzt sich der Kundendeckungsbeitrag zusammen?

☐ a) Zinsen, Tilgung, Gebühren, Sicherheitenwert
☐ b) Zinsmarge, Provisionen, Verbundprovisionen, abzüglich Risikokosten und Betriebskosten
☐ c) Umsatz, Gewinn, Eigenkapital, Rücklagen
☐ d) EAD, PD, LGD, RWA

**F2:** Was bedeutet Expected Loss (EL) in der Konditionenkalkulation?

☐ a) Der maximale mögliche Verlust im Stressfall
☐ b) Der durchschnittlich erwartete Verlust = PD × LGD × EAD
☐ c) Der Verlust, der bereits eingetreten ist und abgeschrieben wurde
☐ d) Der regulatorische Eigenkapitalbedarf nach Basel III

**F3:** Warum ist ein Kredit mit Zins unter dem Richtsatz aus Bankperspektive problematisch?

☐ a) Er ist rechtlich nicht erlaubt
☐ b) Er deckt die Risikokosten nicht – die Bankbeziehung erzeugt einen negativen Deckungsbeitrag
☐ c) Er ist nur dann problematisch, wenn der Kunde auch kein Cross-Selling-Potenzial hat
☐ d) Nur wenn er länger als 5 Jahre läuft

**F4:** Ein Kunde fragt nach einer Konditionssenkung um 20 Basispunkte. Was ist die wertbasierte Antwort?

☐ a) Sofort nachgeben, um den Kunden zu halten
☐ b) Ablehnen und auf Standardkonditionen verweisen
☐ c) Zunächst den Wert der Beziehung (Leistungen, Verlässlichkeit, Verbundleistungen) kommunizieren, dann Konditionsrahmen klären
☐ d) Den Kunden bitten, ein Gegenangebot schriftlich vorzulegen

**F5:** Welcher Optimierungshebel hat den direktesten Einfluss auf den Deckungsbeitrag ohne Gespräch mit dem Kunden?

☐ a) Cross-Selling neuer Produkte
☐ b) Reduzierung der internen Betriebskosten (z.B. Prozessoptimierung, Digitalisierung)
☐ c) Konditionsanpassung nach oben
☐ d) Erhöhung des Kreditvolumens

**F6:** Was beschreibt die Marktzinsmethode (Schierenbeck et al., 2014)?

☐ a) Eine Methode, um Marktpreise für Immobilien zu berechnen
☐ b) Ein Verfahren, das Bankgeschäfte zu Opportunitätskostensätzen des Kapitalmarkts bewertet und so Margen transparent macht
☐ c) Ein Verfahren zur Ermittlung der Leitzinsentwicklung
☐ d) Eine Methode zur Schätzung der Kundenausfallwahrscheinlichkeit

**F7:** Ein Kunde hat bei Ihrer Bank einen Kredit mit negativem Deckungsbeitrag, aber hohe Provisionserträge aus Verbundprodukten. Was ist die richtige Schlussfolgerung?

☐ a) Die Beziehung ist insgesamt nicht profitabel
☐ b) Die Gesamtbeziehung kann trotzdem positiv sein – der Gesamtdeckungsbeitrag (alle Komponenten) entscheidet
☐ c) Der Kredit muss sofort gekündigt werden
☐ d) Verbundprovisionen sind nicht Teil des Kundendeckungsbeitrags

**F8:** Wann ist es sinnvoll, eine Konditionssenkung als Optimierungshebel zu nutzen?

☐ a) Immer, wenn der Kunde danach fragt
☐ b) Nie – Konditionen sollten stabil gehalten werden
☐ c) Wenn Cross-Selling-Potenzial realisiert wird oder der Kundenwert durch Volumenwachstum steigt, das die Senkung kompensiert
☐ d) Nur bei Neukunden, nicht bei Bestandskunden

#### Musterlösungen (Trainer-Version)

| F | Antwort | Lernziel |
|---|---------|---------|
| 1 | b | LZ-1 (DB-Komponenten) |
| 2 | b | LZ-3 (Expected Loss) |
| 3 | b | LZ-3 (risikoadjustiertes Pricing) |
| 4 | c | LZ-5 (wertbasiertes Gespräch) |
| 5 | b | LZ-4 (Optimierungshebel) |
| 6 | b | LZ-1 (Marktzinsmethode) |
| 7 | b | LZ-2 (Ertragsstruktur) |
| 8 | c | LZ-4 (Optimierungsszenarien) |

> **Auswertungshinweis:** 7–8 richtig: Lernziele erreicht | 5–6: Vertiefung empfohlen | unter 5: Wiederholungsgespräch.

---

## 6.3 Kirkpatrick Level 3 – Verhalten (Transferbeobachtung)

*Beobachtungsbogen für FK-Führungskraft – auszufüllen 4–6 Wochen nach dem Modul*

**Mitarbeiter:** ___________________________ | **Beobachter:** ___________________________ | **Datum:** _______________

| Nr | Verhaltensanker | Regelmäßig | Gelegentlich | Noch nicht |
|----|-----------------|:----------:|:------------:|:----------:|
| 1 | Der Berater kennt den ungefähren Deckungsbeitrag seiner wichtigsten Kundenbeziehungen. | ☐ | ☐ | ☐ |
| 2 | Der Berater bereitet Konditionsgespräche mit einer DB-Analyse vor. | ☐ | ☐ | ☐ |
| 3 | Der Berater kommuniziert vor Konditionsverhandlungen den Wert der Bankbeziehung. | ☐ | ☐ | ☐ |
| 4 | Der Berater benennt den größten Ertragsoptimierungshebel je Kunde (Konditionen, Cross-Selling oder Kosten). | ☐ | ☐ | ☐ |
| 5 | Der Berater begründet Konditionsentscheidungen mit Risikokosten-Argumentation. | ☐ | ☐ | ☐ |
| 6 | Der Berater verknüpft Cross-Selling-Aktivitäten mit dem Gesamtdeckungsbeitrag des Kunden. | ☐ | ☐ | ☐ |
| 7 | Der Berater gibt bei Konditionsanfragen nicht sofort nach, sondern fragt zuerst nach dem Hintergrund. | ☐ | ☐ | ☐ |
| 8 | Der Berater dokumentiert Konditionsverhandlungen und deren Ergebnis strukturiert in agree. | ☐ | ☐ | ☐ |

**Stärkster Bereich:** ___________________________________________________________________________

**Entwicklungsfeld:** ___________________________________________________________________________

**Empfehlung:** ☐ Kompetenz M14 erreicht – weiter zu M15/M16 | ☐ Coaching empfohlen | ☐ Auffrischung empfohlen

---

<!-- CONTENT_TRAINER_END -->
